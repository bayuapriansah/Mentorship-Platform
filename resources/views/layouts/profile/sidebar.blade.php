<aside class="w-full bg-[#FAFAFA] absolute z-[10] -top-5 rounded-xl border border-grey p-5">
    <div class="grid grid-cols-12 gap-2 grid-flow-col">
        <div class="col-span-2">
            <button type="button" data-modal-target="notification-modal" data-modal-toggle="notification-modal"
                ...
                @if (notifyStudentCount() > 0)
                    <div
                        ...
                        {{ notifyStudentCount() }}</div>
                @endif
            </button>
        </div>
        <div class="col-span-2">
            <button type="button" data-modal-target="message-modal" data-modal-toggle="message-modal"
                class="relative inline-flex items-center text-sm font-medium text-center text-light-blue rounded-lg hover:text-dark-blue focus:ring-4 focus:outline-none focus:ring-blue-300"
                alt="message">
                <svg class="w-6 h-6" aria-hidden="true" fill="none" stroke="currentColor" stroke-width="1.5"
                    viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
        ...
        <div class="col-end-13">
            <button type="button" data-modal-target="popup-logout" data-modal-toggle="popup-logout"
                class="relative inline-flex items-center text-sm font-medium text-center text-light-blue rounded-lg hover:text-dark-blue focus:ring-4 focus:outline-none focus:ring-blue-300"
                alt="Logout">
                <svg class="w-6 h-6" aria-hidden="true" fill="none" stroke="currentColor" stroke-width="1.5"
                    viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
    ...
    <div class="flex flex-col mt-8 ">
        <div class="mx-auto">
            <img src="{{ $student->profile_picture ? asset('storage/' . $student->profile_picture) : asset('/assets/img/profile-placeholder.png') }}"
                onerror="this.src = `{{ asset('/assets/img/profile-placeholder.png') }}`" alt="Avatar"
                class="w-[100px] h-[100px] rounded-full  mx-auto object-cover ring ring-[#C5CAF3]">
            <p class="mt-4 text-darker-blue text-xl text-center capitalize">{{ $student->first_name }}
                {{ $student->last_name }}</p>
            <p class="mt-2 text-black text-sm text-center">{{ $student->year_of_study }} Year<br>
                {{ $student->study_program }} </p>
            <div class="w-12 h-12 mt-2 mx-auto bg-white rounded-full flex justify-center items-center">
                <img src="{{ $student->institution->logo ? asset('storage/' . $student->institution->logo) : asset('/assets/img/institution-placeholder.png') }}"
                    onerror="this.src = `{{ asset('/assets/img/institution-placeholder.png') }}`" alt="Logo"
            ...
            <div class="flex justify-between">
                ...
                <div class="w-full  relative">
                        ...
                        @foreach ($submissions as $submission)
                            ...
                            @if ($loop->index % 2 == 0)
                                @if ($submission->grade == null)
                                    ...
                                    <img src="{{ asset('assets/img/icon/flag.png') }}" class="absolute top-0"
                                        alt="" style="margin-left: {{ $submission->flag_checkpoint }}%"
                                        data-tooltip-target="tooltip-bottom{{ $tipNumber }}"
                                        data-tooltip-placement="bottom" data-tooltip-trigger="hover">
                                    <div id="tooltip-bottom{{ $tipNumber }}" role="tooltip"
                                        class="absolute z-10 invisible inline-block px-3 py-2 text-black text-xs font-normal border border-light-blue bg-white rounded-lg shadow-sm opacity-0 tooltip">
                                        Task {{ $tipNumber }} <br>
                                        {{ \Carbon\Carbon::parse($submission->created_at)->format('d M Y') }}
                            ...
                            @else
                                @if ($submission->grade == null)
                                    <img src="{{ asset('assets/img/icon/flag.png') }}" class="absolute bottom-0"
                                        alt="" style="margin-left: {{ $submission->flag_checkpoint }}%"
                                        data-tooltip-target="tooltip-bottom{{ $tipNumber }}"
                                        data-tooltip-placement="bottom" data-tooltip-trigger="hover">
                                    <div id="tooltip-bottom{{ $tipNumber }}" role="tooltip"
                    ...
                    <div class="w-full bg-gray-200 rounded-full h-1.5 mb-4 mt-3">
                        <div class="bg-[#11BF61] h-1.5 rounded-full"
                            style="width: {{ round($taskDate) >= 100 ? 100 : round($taskDate) }}%"></div>
                    </div>
            ...
            <div class="flex justify-between">
                ...
                <div class="w-full relative">
                    ...
                    @foreach ($enrolled_projects->where('is_submited', 1) as $enrolled_project)
                        <img data-tooltip-target="tooltip-bottom{{ $tipNumber }}" data-tooltip-placement="bottom"
                            data-tooltip-trigger="hover" src="{{ asset('assets/img/icon/flag.png') }}"
                            class="absolute top-0" alt=""
                            style="margin-left: {{ $enrolled_project->flag_checkpoint >= 95 ? '95' : $enrolled_project->flag_checkpoint }}%">
                    ...
                    <div class="bg-gray-200 rounded-full h-1.5 mb-4 mt-4 ">
                        <div class="bg-[#11BF61] h-1.5 rounded-full "
                            style="width: {{ $dataDate >= 100 ? 100 : $dataDate }}%"></div>
                    </div>
                ...
                @if ($submissionData == null)
                    @if (\Carbon\Carbon::now()->format('Y-m-d') <= $student->end_date)
                        <button data-modal-target="staticModal" data-modal-toggle="staticModal"
                            class="text-sm font-normal text-white bg-primary rounded-full p-2 mx-auto w-3/4">Submit
                            Task</button>
                    @endif
                @else
                    <p class="text-xs mx-16 text-center py-2">You've successfully completed the task on
                        <span id="submitdate"></span>
                    </p>
                    <script>
                        let serverdate = "{{ \Carbon\Carbon::parse($submissionData->created_at)->toIso8601ZuluString() }}";
                        let datesubmit = new Date(serverdate);
                        const formattedDate = datesubmit.toLocaleString('en-IN', {
                            day: 'numeric',
                            month: 'long',
                            year: 'numeric',
                            hour: '2-digit',
                    ...
                    <div
                        class="mx-auto w-full border border-light-blue rounded-xl text-center p-3 flex justify-between items-center">
                        ...
                        @else
                            ...
                            @endphp
                            <a download="submission.{{ $ekstension }}"
                                href="{{ asset('storage/' . $submissionData->file) }}">
            ...
            <i class="fa-solid fa-arrow-up-right-from-square"></i>
                        ...
                        <div class="flex flex-col text-center my-4">
                            ...
                            <div class="flex flex-wrap justify-start pt-2">
                                @foreach ($datasets as $dataset)
                                    <a href="{{ $dataset }}"
                                        class="bg-light-brown hover:bg-dark-brown px-4 py-1 rounded-lg text-white mr-2 mb-2"
                    ...
                    @if ($submissionData->grade)
                        @if ($submissionData->grade->status == 1)
                            <p class="text-dark-blue font-medium text-sm text-center">Status : <span
                                    class="text-[#11BF61]">Complete</span></p>
                        @elseif($submissionData->grade->status == 0)
                            @if (\Carbon\Carbon::now()->format('Y-m-d') <= $student->end_date)
                                <button data-modal-target="staticModal" data-modal-toggle="staticModal"
                                    class="text-sm font-normal text-white bg-darker-blue hover:bg-dark-blue rounded-full p-2 mt-2">Resubmit
                                    Task </button>

                                <p class="text-dark-blue font-medium text-sm text-center">Status : <span
                                        class="text-[#EA0202]">Revise</span></p>
                            @endif
                        @endif
                    @else
                        <p class="text-dark-blue font-medium text-sm text-center">Status : <span
                                class="text-light-brown">In Review</span></p>
                    @endif
                @endif
            @else
                @if ($submissionData == null)
            ...
            @if ($enrolled_projects->where('is_submited', 1)->count() == 5)
                @if ($student->feedback_done == null)
                    ...
                    <button data-modal-target="authentication-modal" data-modal-toggle="authentication-modal"
                        class="text-sm text-center font-normal text-white bg-darker-blue hover:bg-dark-blue rounded-full p-2"
                        type="button">
                        Click Here!
                    </button>
                        ...
                        <div class="relative w-full max-w-md max-h-full">
                            ...
                            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                ...
                                <div class="px-6 py-6 lg:px-8">
                                    <h3 class="mb-4 text-xl font-medium text-gray-900 dark:text-white">Congratulations
                                        on Completing Your Internship!</h3>
                                    <form class="space-y-6" action="{{ route('student.feedback', $student->id) }}"
                                        method="POST">
                                        @csrf
                                        <div>
                                            <label for="feedback"
                                                class="block mb-2 text-sm text-gray-900 dark:text-white">We would be
                                                grateful if you could take a moment to share your feedback or a
                                                testimonial about your time with us.</label>
                                            <textarea id="feedback" name="feedback" rows="4" maxlength="1000"
                                                class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                                placeholder="Write your thoughts here..."></textarea>
                                            <span id="counter">0/1000</span>
                                        </div>
                                        <button type="submit"
                                            class="text-sm text-center font-normal text-white bg-darker-blue hover:bg-dark-blue rounded-full p-2 px-6">Submit
                ...
                @elseif($student->feedback_done == 1)
                    ...
                    <a href="/profile/{{ Auth::guard('student')->user()->id }}/{{ $student->institution->id }}/certificate"
                        class="text-sm text-center font-normal text-white bg-darker-blue hover:bg-dark-blue rounded-full p-2">Download
                        Certificate</a>
                ...
                @endif
            @elseif($total = $totalMonth->sum() < 3 && \Carbon\Carbon::now()->format('Y-m-d') > $student->end_date)
                <p class="text-dark-blue font-medium text-sm text-center my-3">Sorry! You did not meet the requirements
                    to complete the mentorship programs.</p>
