...
@section('content')
    <div class="w-full lg:w-3/4">
        <a href="{{ route('dashboard.submission.show', ['project' => $project->id]) }}"
            class="group block w-max text-lg text-[#6973C6]">
        ...
        <div class="mt-2 flex justify-between items-center">
            ...
            @if ($submission->grade)
                @if ($submission->grade->status == 0)
                    <div
                        class="border-4 border-red-600 text-red-600 rounded-full w-24 h-24 flex items-center justify-center">
                        Revise
                    </div>
                @elseif ($submission->grade->status == 1)
                    <div
                        class="border-4 border-green-600 text-green-600 rounded-full w-24 h-24 flex items-center justify-center">
                        Pass
                    </div>
                @endif
            @endif
    ...
    <div class="mt-10 space-y-10">
        <div class="space-y-4">
            ...
            <div
                class="py-4 px-6 w-3/4 bg-white hover:bg-[#F2F3FD] border border-light-blue rounded-xl flex justify-between">
                    ...
                    <div class="flex flex-wrap justify-start pt-2">
                        @foreach ($datasets as $dataset)
                            <a href="{{ $dataset }}"
                                ...
                                target="_blank">Dataset {{ $no }} <i class="fa-solid fa-chevron-right"></i></a>
                            @php $no++ @endphp
                        @endforeach
                    </div>
                </div>
    ...
    @if (!Auth::guard('customer')->check())
        @if (Auth::guard('web')->check() ||
                Auth::guard('mentor')->user()->id == $submission->student->mentor_id ||
                Auth::guard('mentor')->user()->id == $submission->student->staff_id)
            @if (!$submission->grade)
                @if (optional(Auth::guard('mentor')->user())->institution_id == 0 || Auth::guard('web')->check())
                    <form
                        action="/dashboard/submissions/project/{{ $project->id }}/view/{{ $submission->id }}/adminGrade"
                        method="post">
                        @csrf
                        <div class="px-4 py-2 bg-white rounded-lg border border-light-blue">
                            <textarea id="comment" rows="4" class="w-full px-0 text-sm text-gray-900 bg-white border-0  focus:outline-none"
                                name="message" placeholder="Add Comments (Optional)"></textarea>
                        </div>
                        <div class="flex space-x-6">
                            {{-- data-modal-target="defaultModal" data-modal-toggle="defaultModal" --}}

                            <input type="submit"
                                class="text-white text-sm font-normal bg-[#11BF61] hover:bg-green-600 cursor-pointer px-12 py-3 mt-5  rounded-full"
                                name="pass" value="PASS">

                            <button data-modal-target="popup-modal" data-modal-toggle="popup-modal"
                                class="text-white text-sm font-normal bg-[#AB0606] hover:bg-red-800 cursor-pointer px-12 py-3 mt-5  rounded-full"
                                type="button">
                                NEED REVISION
                            </button>
                                ...
                                <div class="relative w-full h-full max-w-md md:h-auto ">
                                    <div class="relative bg-white rounded-lg border border-light-blue shadow ">
                                        ...
                                        <div class="p-6 text-left">
                                            <p class="text-dark-blue font-medium text-xl">Are you sure you want to mark this
                                                submission as revise submission?</p>
                                            <p class="text-base">Once submission is marked as revise, you can let the
                                                student re-submit the solution.</p>
                                            <br>
                                            <div class="flex space-x-4">
                                                <button type="submit" name="revision" value="revision"
                                                    class="bg-dark-blue w-1/2 rounded-lg text-white py-2">
                                                    Confirm
                                                </button>
                                                <button type="button" data-modal-hide="popup-modal" type="button"
                                                    class="bg-[#B52809] w-1/2 rounded-lg text-white py-2">
                                                    Cancel
                                                </button>
                                            </div>
    ...
    <div class="space-y-2">
        ...
        <button data-modal-target="task-detail-modal" data-modal-toggle="task-detail-modal"
            class="flex items-center gap-2 text-sm text-grey">
                ...
                @if (optional(Auth::guard('mentor')->user())->institution_id == 0 || Auth::guard('web')->check())
                    <div x-data="{ opendetail: false }">
                        <div>
                            ...
                            <button type="button" class="btn btn-primary" x-on:click="opendetail = !opendetail">
                                More Detail
                            </button>
                        </div>
                        ...
                        <div x-show="opendetail">
                            <div>
                                <div>
                                    <div>
                                        <h5>Change Students Grade</h5>
                                    </div>

                                    <div>
                                        <form
                                            action="{{ route('dashboard.submission.changeGrade', ['project' => $project->id, 'submission' => $submission->id]) }}"
                                            method="post">
                                            @csrf

                                            <div class="mb-3">
                                                <textarea id="comment" rows="4" class="form-control" name="messageFeedback"
