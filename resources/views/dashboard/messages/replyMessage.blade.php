...
@section('content')
    ...
    <form action="/dashboard/messages/{{ $injection->id }}/reply/{{ $participant->id }}/sendMessage" method="post"
        id="form-chat" class="text-dark-blue text-lg font-normal" enctype="multipart/form-data">
        @csrf
        <div >
            ...
            <div class="w-full mt-2 mb-4 border border-light-blue rounded-lg  ">
                <div class="bg-white  rounded-t-lg   ">
                    <textarea id="comment" rows="4" class="w-full px-0 text-sm text-gray-900 bg-white border-0  focus:outline-none"
                        name="message" placeholder="Type Here" required></textarea>
                </div>
                <div class="flex items-center justify-between px-3 py-2 bg-white rounded-b-lg">
                    <div class="flex pl-0 space-x-1 sm:pl-2 items-center">
                        <label for="file-chat-input" type="button"
                            class="inline-flex justify-center p-2 text-gray-500 rounded cursor-pointer hover:text-gray-900 ">
                    ...
                    <div>
                        <a href="/dashboard/messages/{{ $injection->id }}/single/{{ $participant->id }}"
                            class="bg-[#B11313] hover:bg-red-800 px-8 py-1 rounded-full text-white text-sm">Cancel</a>
...
@section('more-js')
    <script>
        ...
        fileInput.addEventListener('change', e => {
            fileName.innerHTML =
                `<img src="{{ asset('assets/img/icon/Vector.png') }}" alt=""> ${e.target.files[0].name} <i class="fas fa-times"></i> `
            // You can handle the files here.
            handleUpload(e.target.files)
            let classesToAdd = ["p-2", "border", "border", "hover:bg-gray-50", "rounded-md", "border-dark-blue",
                'p']
