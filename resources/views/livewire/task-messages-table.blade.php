                ...
                <tbody>
                    @foreach ($participants as $participant)
                        <tr class="{{ $loop->iteration % 2 === 0 ? 'bg-[#EBEDFF]' : 'bg-[#F8F8F8]' }}">
                            ...
                            <td class="pr-8 py-2">
                                {{ $this->unreadMessagesCount($participant->id) }}
                            </td>

                            <td class="pr-5 py-2 rounded-e-lg">
                                <a href="{{ route('dashboard.messages.single', ['injection' => $this->projectSection->id, 'participant' => $participant->id]) }}" class="w-max px-3 py-1 bg-primary rounded-lg flex justify-between items-center gap-3 text-white text-sm">
                                    View Messages
                                    <span>></span>
                                </a>
                            </td>
