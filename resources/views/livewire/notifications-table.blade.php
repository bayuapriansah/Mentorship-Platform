                                ...
                                <td class="px-5 py-3 rounded-s-lg rounded-e-lg">
                                    <div class="flex items-baseline">
                                        ...
                                        <div class="max-w-[70%] ml-3">
                                            <p
                                                class="text-{{ $notify_mentor['isRead'] != 1 ? 'dark-blue hover:text-darker-blue' : 'grey hover:text-gray-800' }} font-medium">
                                                There is
                                                {{ $notify_mentor['statusSubmission'] != 'revision' ? 'New Submission' : 'Resubmission' }},
                                                From :
                                                {{ $notify_mentor['studentName'] }} at Section
                                                ({{ $notify_mentor['taskTitle'] }})
                                            </p>
                                        ...
                                        <a href="{{ route('dashboard.notifications.mentor.markAsRead', ['idNotify' => $notify_mentor['idNotify']]) }}"
                                            class=" self-center ml-auto px-3 py-1 bg-primary rounded-lg flex gap-6 text-sm text-white"
                                            onclick="event.preventDefault(); document.getElementById('mark-as-read-form-{{ $notify_mentor['idNotify'] }}').submit();">
                                            View Notification
                                            <span>></span>
                                        </a>

                                        <form id="mark-as-read-form-{{ $notify_mentor['idNotify'] }}"
                                            action="{{ route('dashboard.notifications.mentor.markAsRead', ['idNotify' => $notify_mentor['idNotify']]) }}"
